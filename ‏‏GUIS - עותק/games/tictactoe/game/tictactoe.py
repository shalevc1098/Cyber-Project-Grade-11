...
def restart(my_socket, game_id, screen):
    ...
    pygame.draw.rect(screen, (150, 202, 15), (415,450,100,50))
...
def hanlde_server_response(my_socket, screen, game_id, username, window):
...
# main loop
def start(game_id, player, my_socket, username, window):
    ...
    screen.fill(BG_COLOR)
    draw_lines(screen)
    a = threading.Thread(target=hanlde_server_response, args=(my_socket, screen, game_id, username, window,))
    a.start()
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not game_over:
                    ...
                    if 100 <= mouseX <= 200 and 450 <= mouseY <= 500:
                        my_socket.send(f"tictactoe,end_game,{game_id}".encode())
                    elif 415 <= mouseX <= 515 and 450 <= mouseY <= 500:
                        if not ready:
                            ready = True
                            pygame.draw.rect(screen, (150, 0, 15), (415,450,100,50))
