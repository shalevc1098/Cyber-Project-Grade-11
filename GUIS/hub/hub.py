...
sys.path.insert(0, 'C:/Users/Shalev/Desktop/תיקיות/Python Project/GUIS/games/fourinarow/queue')
import fourinarow_queue
...
from GUIS.hub import hub_support
...
def on_closing(my_socket, root):
    global canbreak
    canbreak = True
    tictactoe_queue.set_can_close(True)
    fourinarow_queue.set_can_close(True)
    my_socket.shutdown(socket.SHUT_RDWR)
    my_socket.close()
    root.destroy()

def vp_start_gui(my_socket, username, geometry = None):
    '''Starting point when module is the main routine.'''
    global val, w, root, canbreak
    root = tk.Tk()
    if geometry == None:
        top = Hub (my_socket, username, top=root)
    else:
        top = Hub (my_socket, username, geometry, root)
    hub_support.init(root, top)
    threading.Thread(target=loop, args=(my_socket,))
...
def open_tictactoe_queue(top, username, my_socket):
    ...
    tictactoe_queue.vp_start_gui(my_socket, username, geometry)

def open_fourinarow_queue(top, username, my_socket):
    global canbreak
    geometry = top.geometry()
    canbreak = True
    top.withdraw()
    importlib.reload(fourinarow_queue)
    fourinarow_queue.vp_start_gui(my_socket, username, geometry)
...
class Hub:
    def __init__(self, my_socket, username, geometry = None, top=None):
        ...
        self.Four_In_A_Row.configure(command=lambda: open_fourinarow_queue(top, self.username, self.my_socket))
