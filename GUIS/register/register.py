...
def vp_start_gui(my_socket):
    ...
    global val, w, root
    root = tk.Tk()
    top = Register (my_socket, root)
    register_support.init(root, top)
    root.protocol("WM_DELETE_WINDOW", lambda: on_closing(my_socket, root))
    root.mainloop()

def register_command(top, my_socket, username, password, label):
    if (username and password) != "":
        my_socket.send(f"db,register,{username},{password}".encode())
...
class Register:
    def __init__(self, my_socket, top=None):
        ...
        self.Register_Button.place(relx=0.25, rely=0.444, height=24, width=307)
        ...
        self.Register_Label = tk.Label(top)
        self.Register_Label.place(relx=0.45, rely=0.52, anchor="center")
        self.Register_Label.configure(background="#d9d9d9")
        self.Register_Label.configure(foreground="#000000")

        self.Register_Button.configure(command=lambda: register_command(top, self.my_socket, self.Username_Entry.get(), self.Password_Entry.get(), self.Register_Label))