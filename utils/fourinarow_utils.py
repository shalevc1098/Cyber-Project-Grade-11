...
def check_request(command = None, client1 = None, client2 = None, sender = None, figure = None, game_id = None, row = None, col = None):
    global last_game_id
    if command == "new_game":
        active_games[last_game_id] = FourInARow(client1, client2, last_game_id)
        last_game_id += 1
        return "fourinarow,new_game"
    elif command == "check_turn":
        sender.send(str(active_games[game_id].get_turn()).encode())
        return "fourinarow,check_turn"
...
class FourInARow:
    def __init__(self, client1, client2, game_id):
        ...
        self.client1_figure = 1
        self.client2_figure = 2
        self.current = 1
        self.ready = []
        client1.send(f"start_fourinarow,{game_id},{self.client1_figure}".encode())
        client2.send(f"start_fourinarow,{game_id},{self.client2_figure}".encode())
